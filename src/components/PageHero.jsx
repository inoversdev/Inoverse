...
export default function PageHero({ eyebrow, heading, accent, lede }) {
  return (
    <section className="relative mx-auto max-w-7xl px-6 pt-40 pb-16 lg:px-10 lg:pt-48">
      <p data-parallax="-0.1" className="mb-3 text-xs font-medium uppercase tracking-[0.2em] text-ember-500">{eyebrow}</p>
      <SplitHeading
        as="h1"
        text={heading}
        accent={accent}
        className="font-display text-[2.75rem] font-semibold leading-[1.02] tracking-[-0.03em] text-star-100 sm:text-6xl lg:text-6xl"
      />
      {lede ? (
