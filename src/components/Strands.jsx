// ─── Strands — React Bits component (reactbits.dev, MIT), integrated
// for Inovers v2 as the LIGHT-MODE band (Mat's call 2026-08-12: use
// Strands for the elegant white mode instead of WebThreads — its
// glow-based shader reads bright on light backgrounds, no dark tint).
//
// Adapted from the official source with Inovers additions:
//   - prefers-reduced-motion renders ONE static frame (no rAF loop)
//   - depth:false renderer (prevents the opaque-backing dark tint)
//   - IntersectionObserver pause + hidden-tab pause
//   - React-19-safe cleanup
//
// The colors prop drives the palette (ember family for Inovers light
// mode). glass=false keeps it a pure strand weave (no glass ball).
...
// glass path kept for API parity (Mat's config uses glass=false — the
// strand weave renders directly to the canvas, no glass ball).
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      depth: false, // no opaque backing store — prevents the dark tint
      stencil: false,
      antialias: true,
      dpr: Math.min(window.devicePixelRatio || 1, getDeviceProfile().dprCap),
    })
    ...
    const glassMesh = new Mesh(gl, { geometry, program: glassProgram, frustumCulled: false })
