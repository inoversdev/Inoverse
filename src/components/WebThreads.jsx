// ─── WebThreads — React Bits component (reactbits.dev, MIT) — integrated
// for Inovers v2 as the COSMIC CLOUDS band. Exact official source, with
// one Inovers addition: prefers-reduced-motion renders a single static
// frame instead of running the rAF loop.
...
void main() {
  ...
  // STRAIGHT alpha output: RGB is the un-premultiplied thread color.
  // The renderer runs with premultipliedAlpha:false, so the browser
  // composites these as-is over the page — transparent pixels stay
  // fully transparent and never tint the background (this kills the
  // dark veil the premultiplied path left on light paper).
  vec3 outRgb = col;
...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    // Reduced motion: draw one static frame, skip the loop.
    if (reduceMotion) {
