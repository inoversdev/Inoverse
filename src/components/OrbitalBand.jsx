...
// ─── OrbitalBand — the organizational chart as an orbiting system ───
// Relocated out of the home About section (SpaceAbout) onto /crew, where
// it now opens the page above the people grid. Department-level only: no
// individuals orbit here, just the seats and teams from ORG_CHART — the
// same taxonomy the filter chips below it are derived from.
//
// Pure CSS 3D motion (rings, gyroscope precession, Saturn shading); GSAP
// only handles the scroll entrance, and it all lives inside a
// gsap.context scoped to rootRef so every ScrollTrigger this component
// creates dies with it on unmount. That matters here specifically:
// these triggers used to belong to a home-page component, and a stale
// trigger pointing at a node that left the DOM is exactly the leak this
// relocation could have introduced.
...
export default function OrbitalBand() {
  ...
  useEffect(() => {
    ...
    // inline "from" states — the scene renders at its natural opacity and
    // the blanket CSS rule in index.css freezes the ambient spin.
