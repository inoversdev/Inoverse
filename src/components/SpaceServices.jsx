  ...
  return (
    <section id="services" ref={rootRef} className="relative mx-auto max-w-7xl px-6 py-28 lg:px-10">
      <div className="mb-16 max-w-2xl">
        <p className="mb-3 text-xs font-medium uppercase tracking-[0.3em] text-ember-300">Services</p>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {SERVICES.map((s) => (
          ...
          >
            <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-xl border border-ember-500/30 bg-ember-500/10 text-ember-300 transition-colors group-hover:text-ember-200">
              {ICONS[s.icon] || ICONS.code}
            </div>
            <h3 className="font-display text-2xl text-star-100">{s.title}</h3>
            <p className="mt-1 text-sm font-semibold uppercase tracking-wider text-ember-300">{s.promise}</p>
      ...
      <div className="mt-20 grid grid-cols-2 gap-6 lg:grid-cols-4">
        {STATS.map((s) => (
          <div key={s.label} className="glass rounded-2xl p-6 text-center">
            <p className="font-display text-4xl font-light text-star-100">
